...
struct task_section *task_add_section(struct task_table_entry *t, char type, size_t size) {
  struct task_section *s = (struct task_section *)tkalloc(sizeof(struct task_section), "task section desciptor", 1);
  s->type = type;
  s->size = ALIGN(size, 0x1000);
  s->base = tkalloc(s->size, "task section data", 1);
